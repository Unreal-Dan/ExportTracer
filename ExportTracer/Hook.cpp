...
bool Hook::install_hook(bool do_callback, uint32_t num_args)
{
    ...
    m_ret_amt = num_args * sizeof(uintptr_t);
...
bool Hook::calculate_asm_length()
{
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        debug("%02x %02x %02x", curins->bytes[0], curins->bytes[1], curins->bytes[2]);
        debug("%s", curins->mnemonic);
...
bool Hook::create_trampoline(bool do_callback)
{
    ...
    if (do_callback) {
        ...
        m_trampoline_len += (uintptr_t)m_asm_len +  // size of the relocated opcodes
        ...
        for (j = 0; j < x86->op_count; j++) {
            ...
            if (op->type == X86_OP_MEM) {
                ...
                if (is_conditional_jump(curins->bytes, curins->size)) {
                    ...
                    uintptr_t cond_ptr = to + conditional_area + (num_conditionals * ABS_JUMP_SIZE);
...
{
    if (to < from) {
        return 0 - (from - to) - ins_size;
    }
    return to - (from + ins_size);
}
